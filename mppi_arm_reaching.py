...
class ReacherMPPI:
    ...
    def command(self, env):
        ...
        epsilon = self.sigma * \
            np.random.randn(self.K, self.horizon, self.action_dim)
        traj = []
        for k in range(self.K):
            # for k in tqdm(range(self.K)):
            obs = self.sim_env.set_state(env.data.qpos, env.data.qvel)
            for h in range(self.horizon):
                obs, _, _, info = self.sim_env.step(
                    self.u[h] + epsilon[k, h])
                reward = 10 * info['reward_dist'] + info['reward_ctrl']
                sequence_cost[k] += -reward + self.l * \
                    self.u[h].dot(epsilon[k, h]) / self.sigma
        ...
        self.u += np.sum(epsilon * np.tile(importance_weight,
                         (self.action_dim, self.horizon, 1)).T, axis=0)
