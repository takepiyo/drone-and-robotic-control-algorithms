    ...
    def command(self, x, t, x_array):
        ...
        for k in range(self.K):
            ...
            for h in range(self.horizon):
                state, cost, done, _ = self.env.step(
                    self.u[h][0] + epsilon[k, h])
                x = state[0]
                sequence_cost[k] += cost + np.sqrt(np.square(x)) + \
                    self.l * self.u[h][0] * epsilon[k, h] / self.sigma

                if done or h == self.horizon - 1:
                    sequence_cost[k] += self.env.terminate_cost(x) if self.env.n + \
                        1 == self.env.max_n else 0
...
if __name__ == "__main__":
    ...
    ctrl = DoubleSlit1DMPPI(
        sim_env, 1000, horizon=50, sigma=4.0, l=1, plot_per_iteration=40)
